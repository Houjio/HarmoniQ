...
from harmoniq.db.schemas import SQLBase, EolienneCreate, EolienneParcCreate
...
def test_read_all_eolienne(db: Session):
    result = read_all_eolienne(db)
    assert len(result) > 0


def test_read_eolienne_by_id(db: Session):
    result = read_eolienne_by_id(db, 1)
...
def test_delete_eolienne(db: Session):
    ...
    assert result["message"] == "Instance of Eolienne deleted successfully"
